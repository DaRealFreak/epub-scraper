...
func ReadConfigurationFile(fileName string) (novelConfig *NovelConfig, err error) {
	...
	mergeSourceConfigSiteConfig(novelConfig)
...
func mergeSourceConfigSiteConfig(novelConfig *NovelConfig) {
		...
		for _, site := range novelConfig.Sites {
			if source.Toc != nil {
				...
				if tocURL.Host == site.Host {
					updatePagination(&source.Toc.Pagination, &site.Pagination)
					updateChapterContent(&source.Toc.ChapterContent, &site.ChapterContent)
				}
			}
			if source.Chapter != nil {
				...
				raven.CheckError(err)
				if tocURL.Host == site.Host {
					updateChapterContent(&source.Chapter.ChapterContent, &site.ChapterContent)
				}
			}
		}
	}
...
func updatePagination(sourceConfig *Pagination, siteConfig *Pagination) {
...
func updateChapterContent(sourceConfig *ChapterContent, siteConfig *ChapterContent) {
